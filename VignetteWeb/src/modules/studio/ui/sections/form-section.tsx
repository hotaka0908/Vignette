...
const FormSectionSuspense = ({ videoId }: FormSectionProps) => {
  ...
  const generateDescription = trpc.videos.generateDescription.useMutation({
    onSuccess: (data) => {
      ...
      utils.studio.getMany.invalidate();
      utils.studio.getOne.invalidate({ id: videoId });
      toast.success("Gemini description generated");
    },
    onError: () => {
      toast.error("Something went wrong");
    },
  ...
  const generateTitle = trpc.videos.generateTitle.useMutation({
    onSuccess: (data) => {
      ...
      utils.studio.getMany.invalidate();
      utils.studio.getOne.invalidate({ id: videoId });
      toast.success("Gemini title generated");
    },
    onError: () => {
      toast.error("Something went wrong");
    },
  });
  const generateMetadata = trpc.videos.generateMetadata.useMutation({
    onSuccess: (data) => {
      form.reset(data);
      utils.studio.getMany.invalidate();
      utils.studio.getOne.invalidate({ id: videoId });
      toast.success("Gemini metadata generated");
    },
    onError: () => {
      toast.error("Gemini metadata generation failed");
    },
  });
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
            ...
            <div className="flex items-center gap-x-2 flex-wrap">
              <Button
                ...
                onClick={() => generateMetadata.mutate({ id: videoId })}
              >
                {generateMetadata.isPending ? (
                  <Loader2Icon className="animate-spin size-4 mr-1" />
                ) : (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            <div className="space-y-8 lg:col-span-3">
              ...
              />
              {(video.aiSummary || (video.aiTags && video.aiTags.length > 0)) && (
                <div className="rounded-xl border bg-muted/40 p-4 space-y-3">
                  ...
                  {video.aiTags && video.aiTags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {video.aiTags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-background border px-2 py-0.5 text-xs"
                        >
                          #{tag}
                        </span>
                      ))}
                  ...
                  {video.aiSafetyLabel && (
                    <p className="text-xs text-muted-foreground">
                      Safety: {video.aiSafetyLabel}
                      {video.aiSafetyReason ? ` — ${video.aiSafetyReason}` : ""}
                    </p>
                  )}
                </div>
              )}
